...
export class GoogleFinanceScraper {
  ...
  async scrape(symbol: string): Promise<GoogleFinanceQuote> {
    ...
    // this is an example for a not so good practice
    // we use axios here directly
    // instead of using the HttpService injected from NestJS's HttpMoulde
    // see a better working exampke in how user-toekns consumes the auth
    // microservice
